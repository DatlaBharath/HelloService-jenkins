        ...
        stage('Deploy to Kubernetes') {
            steps {
                script {
                    ...
                    sh 'ssh -i /var/test.pem -o StrictHostKeyChecking=no ubuntu@3.6.87.39 "kubectl apply -f -" < deployment.yaml'
                    sh 'ssh -i /var/test.pem -o StrictHostKeyChecking=no ubuntu@3.6.87.39 "kubectl apply -f -" < service.yaml'
                }
            }
        }
    }
