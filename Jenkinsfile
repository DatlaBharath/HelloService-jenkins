        ...
        stage('Deploy to Kubernetes') {
            steps {
                script {
                    ...
                    sh 'ssh -i /var/test.pem -o StrictHostKeyChecking=no ubuntu@3.110.62.20 "kubectl apply -f -" < deployment.yaml'
                    sh 'ssh -i /var/test.pem -o StrictHostKeyChecking=no ubuntu@3.110.62.20 "kubectl apply -f -" < service.yaml'
                }
            }
        }
    }
