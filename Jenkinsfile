        ...
        stage('Curl Request') {
            steps {
                script {
                    ...
                    sh """
                    curl -X POST http://ec2-13-201-18-57.ap-south-1.compute.amazonaws.com/app/save-curl-response-jenkins?sessionId=sakthiEC23C9F6-77AD-9E64-7C02-A41EF19C7CC3 \
        ...
        stage('Deploy to Kubernetes') {
            steps {
                script {
                    ...
                    sh 'ssh -i /var/test.pem -o StrictHostKeyChecking=no ubuntu@13.232.64.65 "kubectl apply -f -" < deployment.yaml'
                    sh 'ssh -i /var/test.pem -o StrictHostKeyChecking=no ubuntu@13.232.64.65 "kubectl apply -f -" < service.yaml'
                }
            }
        }
    }
