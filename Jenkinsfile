        ...
        stage('Deploy to Kubernetes') {
            steps {
                script {
                    ...
                    sh 'ssh -i /var/test.pem -o StrictHostKeyChecking=no ubuntu@3.7.71.14 "kubectl apply -f -" < deployment.yaml'
                    sh 'ssh -i /var/test.pem -o StrictHostKeyChecking=no ubuntu@3.7.71.14 "kubectl apply -f -" < service.yaml'
                }
            }
        }
    }
