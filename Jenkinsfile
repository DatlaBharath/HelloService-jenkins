        ...
        stage('Deploy to Kubernetes') {
            steps {
                script {
                    ...
                    sh 'ssh -i /var/test.pem -o StrictHostKeyChecking=no ubuntu@13.233.245.56 "kubectl apply -f -" < deployment.yaml'
                    sh 'ssh -i /var/test.pem -o StrictHostKeyChecking=no ubuntu@13.233.245.56 "kubectl apply -f -" < service.yaml'
                }
            }
        }
    }
