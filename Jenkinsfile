pipeline {
    ...
    environment {
        dockerImageName = "helloservice-jenkins"
    }
    stages {
        stage('Checkout') {
            steps {
        ...
        stage('Build Docker Image') {
            steps {
                script {
                    def repoName = dockerImageName.toLowerCase()
                    def imageName = "${dockerUsername}/${repoName}:${env.BUILD_NUMBER}"
  ...
  template:
    ...
    spec:
      ...
      - name: ${dockerImageName}
        image: ${dockerUsername}/${dockerImageName}:${env.BUILD_NUMBER}
        ports:
        - containerPort: 5000
"""
                    def serviceYaml = """
